...
import requests
from .aspnet import extract_tokens, form_action_url, find_login_fields, debug_list_inputs

PORTAL_URL = "https://portal.amhp.com.br/"
AMHPTISS_URL = "https://amhptiss.amhp.com.br/Default.aspx"


def login_portal(session: requests.Session,
                 username: str,
                 password: str,
                 timeout: int = 30,
                 user_field_override: str = None,
                 pass_field_override: str = None,
                 submit_name_override: str = None):
    """
    GET portal -> tokens -> (override ou heurística) -> POST credenciais.
    Retorna (ok, response_post, reason_text_ou_debug).
    """
    ...
    if user_field_override and pass_field_override:
        user_field, pass_field = user_field_override, pass_field_override
        submit_name, submit_value = submit_name_override, "Entrar"
    else:
        user_field, pass_field, submit_name, submit_value = find_login_fields(soup)

    if not user_field or not pass_field:
        dbg = debug_list_inputs(soup)
        return False, r_get, (
            "Não identifiquei campos de usuário/senha. Informe names manualmente nos campos Avançados.\n\n" + dbg
        )
    ...
    ok = (
        ("sair" in html or "logoff" in html or "minha conta" in html)
        or ("senha" not in html and "usuario" not in html and "usuário" not in html and "login" not in html)
    )
...
def access_amhptiss(session: requests.Session, timeout: int = 30):
    """
    GET no AMHPTISS com a mesma sessão autenticada.
    Retorna (ok, response).
    """
